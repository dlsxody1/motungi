...
export const color = {
  brand: {
     ...
     * 메인 로즈 — 주요 CTA·강조.
     *
     * 2026-08-06 색약(CVD) 대응으로 명도를 내렸다(#d42f4a → #9e2b41).
     * 구 값은 흰 글씨 대비가 정상 시각 4.88:1로 AA(4.5:1)에 겨우 걸쳤고,
     * 색약 시뮬레이션(Brettel/Viénot)을 통과시키면 deutan 4.19:1 · tritan 2.89:1로
     * **AA 아래로 떨어졌다**. 원인은 "빨강이라서"가 아니라 명도 마진이 없어서다 —
     * CVD는 색상 정보를 명도로 붕괴시키는데, 붕괴 후 남는 명도가 부족했다.
     *
     * ⚠️ 그래서 hue는 옮기지 않았다. 같은 로즈에서 명도만 내리면 해결되고,
     *    노을(sun·purple과의 그라데이션) 정체성도 그대로 유지된다.
     *    현 값 흰 글씨 대비: 정상 7.30 · protan 9.62 · deutan 6.49 · tritan 4.81 (전 유형 AA).
     *    이 값을 밝게 되돌리려면 반드시 CVD 3종을 재측정할 것.
     *
     * 2026-08-06(2차) **브랜드를 로즈에서 따뜻한 노을 보라로 전환.** 로즈 #9e2b41은 AA를
     * 통과했지만 최저가 tritan 4.81:1로 기준선(4.5)에 가까웠고, "빨간 글씨·아이콘"이라는
     * 인상 자체가 색약 사용자에게 부담이었다. 보라 #6b3d6e는 최저 7.59:1로 여유가 넓다.
     *
     * ⚠️ 차가운 보라(파랑 섞인 #6e4e9c 계열)가 아니라 **자홍기 있는 따뜻한 노을 보라**다.
     *    노을 스펙트럼(남보라→노을보라→딥로즈→코랄→황금오렌지)의 한 지점이라
     *    sun·rose와 같은 계열로 읽힌다. 파랑 쪽으로 옮기면 밤/AI퍼플이 되니 금지.
     *
     * 랜딩(마케팅)은 예외적으로 구 로즈를 유지한다 — globals.css의 `--color-primary-landing`.
     */
    primary: "#6b3d6e",
    /** 진한 노을 보라 — hover/press. 흰 글씨 대비 정상 9.88 · deutan 9.52 · tritan 8.97 */
    primaryDeep: "#5d3460",
    /**
     * 딥로즈 — 노을 스펙트럼의 붉은 끝. 가격·마감처럼 **진짜 강조**에만 쓴다(CTA 아님).
     * 흰 글씨 대비 최저 tritan 4.81:1로 AA는 통과하나 마진이 좁으니 큰 면적에만.
     */
    rose: "#9e2b41",
    /** 연한 보라 틴트 — 배경·칩. primary(#6b3d6e)에 맞춰 로즈 틴트에서 옮겼다 */
    tint: "#f3ebf4",
    /**
     * 황금 오렌지 — 노을의 밝은 끝. 히어로 그라데이션·로고.
     * ⚠️ sun·coral은 흰 글씨를 못 얹는다(1.99:1 / 3.04:1). 면·보더·아이콘 전용.
     */
    sun: "#e8834a",
    /** 코랄 — 노을의 중간. sun과 같이 면·보더·아이콘 전용(흰 글씨 3.04:1) */
    coral: "#c74b3e",
    /** 남보라 — 노을의 가장 깊은 끝. 흰 글씨 최저 12.32:1 */
    duskDeep: "#3d2b56",
    /**
     * @deprecated 옛 차가운 보조 퍼플(#6e4e9c). 파랑이 섞여 밤/AI퍼플로 읽혔다.
     * 남은 그라데이션 참조가 깨지지 않도록 duskDeep을 가리키게 두고, 옮기는 대로 지운다.
     */
    purple: "#3d2b56",
    /** 민트 — 지원금/정책 강조 */
    mint: "#1e6e64",
   ...
   *
   * 2026-08-06(2차) 배경을 반 단계 눌렀다(#ffffff → #fafaf9). 베이지 폐기 후
   * bg === surface === 순백이 되면서 카드가 배경에서 전혀 분리되지 않았다(대비 1.00:1).
   * 폐기한 웜 아이보리(#f4f0eb)로 되돌아간 게 아니라 **중립 회색(chroma 0)으로 아주 살짝**이다 —
   * 웜 언더톤이 브랜드 로즈와 충돌했던 옛 문제는 재발하지 않는다.
   *
   * ⚠️ bg와 surface의 차이는 1.04:1로 그 자체로는 미미하다. 배경색 하나만으로 면을 나누려
   *    하지 마라. 카드·박스는 여전히 `border-line-alt` + `shadow-card`를 함께 써야 하고,
   *    배경은 그 둘을 **거들 뿐**이다(DESIGN.md 박스 규칙).
   *    회색 면(gray100/gray200)은 **스켈레톤·호버·트랙처럼 "일시적 상태"에만** 쓴다.
   */
  neutral: {
    /** 앱 배경 — 흰 카드가 뜨도록 반 단계 누른 중립. surface(#ffffff) 대비 1.04:1 */
    bg: "#fafaf9",
    /** 카드/표면 흰색 */
    surface: "#ffffff",
    /**
