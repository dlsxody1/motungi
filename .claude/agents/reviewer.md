...
## 할 일
0. **헛도는 밤 감지 (리포트 쓰기 전 먼저).** `ls docs/nightly/` 최근 노트들을 열어 **연속 report-only 횟수**를 센다. 2회 이상이면 TL;DR 최상단에 `⚠️ N일 연속 report-only — 파이프라인이 헛돌고 있다. 사람 개입 필요.`를 박는다. (선례: 2026-07-23~07-30 8일 연속 헛돌았는데 아무도 몰랐다.) 3회 이상이면 무엇이 밤을 막고 있는지 진단을 리포트에 쓴다.
1. dev에 쌓인 이번 밤 diff를 최종 리뷰: 정합성·범위·스타일·리스크·시크릿 노출. 놓친 점을 리포트에 명시.
   - **커밋 메시지 규율(추적성)**: Conventional Commits(`feat(web):`·`fix(core):`) + **본문이나 제목에 백로그 id(`M-NNN`)를 넣는다.** id가 없으면 `git log`에서 이슈를 역추적할 수 없다 — 리포트에만 있으면 코드에서 이슈로 가는 길이 없다.
2. **야간 리포트**를 `docs/nightly/nightly-YYYY-MM-DD.md`(UTC)로 작성해 **dev에 커밋**한다. **리포트 본문은 반드시 한글로 작성한다**(명령 출력·코드·파일 경로는 원문 유지). 구조:
   ...
   ## TL;DR            (클러스터 이슈 id들, 나간 것/되돌린 것, pass/fail 2~3줄)
                       (연속 report-only 2회 이상이면 여기 최상단에 ⚠️ 경고를 먼저 박는다)
