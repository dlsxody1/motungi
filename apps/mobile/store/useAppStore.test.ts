/**
 * useAppStore(mobile wrapper) 계약 테스트.
 *
 * 상태 전이 로직(setAnchor/toggleSaved 등)은 @motungi/core의
 * packages/core/src/store.test.ts가 이미 검증하므로 여기서 다시 다루지 않는다.
 * 이 파일은 mobile 전용 결선만 검증한다:
 *   1) createAppStore에 AsyncStorage/supabase 어댑터가 실제로 주입되는지
 *   2) apps/mobile/lib/auth.ts가 의존하는 getState/setState/subscribe 정적
...
describe("useAppStore (mobile 결선)", () => {
  ...
  it("getState()는 게스트 기본 상태를 반환한다(AsyncStorage 주입 확인)", () => {
