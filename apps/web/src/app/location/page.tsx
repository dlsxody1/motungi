      ...
      <DesktopShell active="home" variant="marketing" footer={false}>
        <WebContainer className="py-14">
          <div className="mx-auto max-w-[560px]">
            ...
            <button className="mt-8 flex w-full items-center gap-4 rounded-[18px] bg-surface p-5 text-left shadow-web transition-shadow hover:shadow-web-lift">
              <span className="grid size-12 place-items-center rounded-full bg-tint text-primary">
                <LocationIcon size={24} />
              </span>
              <span className="flex-1">
                <span className="block text-[16px] font-bold text-ink">현재 위치로 찾기</span>
                <span className="block text-[13px] text-muted">위치 권한 → 행정동 자동 설정</span>
              </span>
            ...
            <Link href="/diagnosis" className="mt-8 block">
              <button className="tap-safe flex h-[56px] w-full items-center justify-center rounded-xl bg-primary text-[17px] font-bold text-white transition-colors hover:bg-primary-deep">
                {selected}으로 시작하기
              </button>
            </Link>
          </div>
